...
namespace Debenhams.Activities
{
	...
	public class ActLBStoreList : Activity
	{
		...
		{
			var items = ItemRepository.GetLoadDetail(txtloadnum.Text);
			lvpo.Adapter = new AdpLBListDetail(this, items);
		}


		private void lvpo_ItemClicked(object sender, AdapterView.ItemClickEventArgs e)
		{
			var item = ((AdpLBListDetail)lvpo.Adapter).GetItemDetail(e.Position);
			var intent = new Intent();
			tblLoadListDetail loadlist = new tblLoadListDetail();
			if (item.status == "Open") 
			{
				loadlist.id = item.id;
				...
				ItemRepository.UpdateLoadListDetail (loadlist);
			}
			intent.SetClass(this, typeof(ActLBStoreListBox));
		...
		private void btnDone_Clicked()
		{
			var builder = new AlertDialog.Builder(this);
			builder.SetTitle(GlobalVariables.GlobalMessage);
			var chkbox = ItemRepository.ChkLoad (txtloadnum.Text);
			if (chkbox.Count()==0) 
			{
			...
			else
			{
				...
				builder.SetMessage("Finish Loading?.\nThe following T.L Number is not Completed\n"+ tl);
			}
		...
		private async void YesDialog_Clicked(object sender, DialogClickEventArgs args)
		{
			...
			try
			{
				...
				var boxes= ItemRepository.ChkloadList123(txtloadnum.Text);
				foreach (var i in boxes) 
				{
					...
					await (ApiConnection1.ApiDebsUpdateData (GlobalVariables.GlobalUrl + "/UpdateLoadingBoxStatus/" + i.move_doc + "/" + i.box_code + "/" + stat));
				}
				await (ApiConnection1.ApiDebsUpdateData (GlobalVariables.GlobalUrl + "/UpdateLoadingStatus/" + txtloadnum.Text + "/" + txtdate.Text));

				tblLoadList item = new tblLoadList();
				var load = ItemRepository.GetLBListFirst (txtloadnum.Text);
				item.id = load.id;
				ItemRepository.DeleteLoadListDetail(load.load_id);

				foreach (var i in boxes) 
				{
					ItemRepository.DeleteLoadListDetailBox1(i.move_doc);
				}
				ItemRepository.DeleteLBList (item);
