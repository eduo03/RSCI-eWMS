...
namespace Debenhams.Activities
{
	...
	public class ActLoading : Activity
	{
		...
		protected override void OnCreate (Bundle savedInstanceState)
		{
			...
			SetContentView (Resource.Layout.LayLoadingMenu);
		...
		public async void ApiRPolist()
		{
			...
			try
			{
				await ApiConnection1.ApiNewLoadingList(GlobalVariables.GlobalUrl +"/NewLoadingList/"+ GlobalVariables.GlobalUserid);
		...
		{
			var items = ItemRepository.GetLoadList1(txtsearch.Text);
			lvpo.Adapter = new AdpLoading(this, items);
		...
		private void lvpo_ItemClicked(object sender, AdapterView.ItemClickEventArgs e)
		{
			...
			var item = ((AdpLoading)lvpo.Adapter).GetItemDetail(e.Position);
			var intent = new Intent();
			if (item.status == "Open") 
			{
				loadlist.id = item.id;
			...
			intent.SetClass(this, typeof(ActLoadingDetail));
