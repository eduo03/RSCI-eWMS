...
namespace Debenhams.Activities
{
	...
	public class ActPSTList : Activity
	{
		...
		protected override void OnCreate (Bundle savedInstanceState)
		{
			...
			SetContentView (Resource.Layout.LayPSTList);
			...
			MenuPickingST.Click += (sender, e) => {leftdrawer ();};
		...
		private async void ApiPTLlist()
		{
			...
			try
			{
				await ApiConnection1.ApiPSTList(GlobalVariables.GlobalUrl +"/PSTList/"+GlobalVariables.GlobalUserid);
				Toast.MakeText (this,"Downloading TL Successfully.", ToastLength.Long).Show ();
				refreshItems ();
			}
			catch(Exception ex) 
			{
		...
		{
			var items = DBConnection.GetPRStList(txtsearch.Text);
			lvpo.Adapter = new AdpPStList(this, items);
		...
		{
			var item = ((AdpPStList)lvpo.Adapter).GetItemDetail(e.Position);
			var intent = new Intent();
			intent.SetClass(this, typeof(ActPSTBox));
			intent.PutExtra("id",item.id.ToString());
			intent.PutExtra("move_doc", item.mts_no);
			intent.PutExtra("store_id", item.location_id);
			intent.PutExtra("store_name", item.location_name);
