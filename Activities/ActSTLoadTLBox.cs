...
namespace Debenhams.Activities
{
	...
	public class ActSTLoadTLBox : Activity
	{
		...
		tblLoadingSTListDetail loading =new tblLoadingSTListDetail();

		protected override void OnCreate (Bundle savedInstanceState)
		{
			base.OnCreate (savedInstanceState);
			SetContentView (Resource.Layout.LaySTLoadBox);
		...
		{
			var items = ItemRepository.GetLoadSTDetail1(txtloadnum.Text);
			lvpo.Adapter = new AdpLoadingSTDetail(this, items);
		}


		private void lvpo_ItemClicked(object sender, AdapterView.ItemClickEventArgs e)
		{
			var item = ((AdpLoadingSTDetail)lvpo.Adapter).GetItemDetail(e.Position);
			if (item.status == "Loaded") 
			{
				...
				builder.SetPositiveButton ("Yes", delegate {
					...
					ItemRepository.UpdateLDetailBox1ST (loading);
					refreshItems();
				});
		...
		private void ScanUPC()
		{
			if (Convert.ToInt32 (txtscanBox.Text.Length) > 1) 
			{
				...
				{
					var scanItem = ItemRepository.ChkLBoxSTExist1 (txtloadnum.Text, txtscanBox.Text);
					if (scanItem != null) 
					{
		...
		private void btnDone_Clicked()
		{
			var builder = new AlertDialog.Builder(this);
			builder.SetTitle(GlobalVariables.GlobalMessage);
			var chkbox = ItemRepository.ChkLoadingST (txtloadnum.Text);
			if (chkbox.Count()==0) 
			{
		...
		private async void YesDialog_Clicked(object sender, DialogClickEventArgs args)
		{
			...
			try
			{
				var boxes= ItemRepository.ChkLoadingboxST(txtloadnum.Text);
				foreach (var i in boxes) 
				{
					...
					await (ApiConnection1.ApiDebsUpdateData (GlobalVariables.GlobalUrl + "/UpdateNewLoadingSTBoxStatus/" + i.load_id + "/" + i.box_code + "/" + stat));
				}
				await (ApiConnection1.ApiDebsUpdateData (GlobalVariables.GlobalUrl + "/UpdateLoadingSTStatus/" + txtloadnum.Text + "/" + txtdate.Text));

				ItemRepository.DeleteNewLoadSTListDetail(txtloadnum.Text);
				ItemRepository.DeleteNewLoadSTList(txtloadnum.Text);
